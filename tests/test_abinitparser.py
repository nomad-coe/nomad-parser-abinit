...
def test_scf(parser):
    ...
    assert sec_run.time_run_date_start.magnitude == pytest.approx(1.46712528e+09)
    ...
    assert sec_run.section_basis_set_cell_dependent[0].basis_set_planewave_cutoff.magnitude == pytest.approx(3.48779578e-17)

    sec_method = sec_run.section_method[0]
    assert sec_method.scf_max_iteration == 10.
    assert sec_method.scf_threshold_energy_change.magnitude == pytest.approx(4.35974472e-24)
    ...
    assert sec_system.atom_positions[1][1].magnitude == pytest.approx(1.346756e-10)
    assert sec_system.lattice_vectors[2][0].magnitude == pytest.approx(2.693512e-10)

    sec_scc = sec_run.section_single_configuration_calculation[0]
    assert sec_scc.energy_total.magnitude == pytest.approx(-3.86544728e-17)
    assert np.max(sec_scc.atom_forces_raw.magnitude) == 0.
    assert sec_scc.stress_tensor[2][2].magnitude == pytest.approx(-5.60539974e+08)
    assert sec_scc.energy_reference_fermi[0].magnitude == pytest.approx(8.4504932e-19)
    assert sec_scc.x_abinit_energy_kinetic == pytest.approx(1.3343978e-17)
    assert len(sec_scc.section_scf_iteration) == 5
    sec_eig = sec_scc.section_eigenvalues[0]
    assert sec_scc.section_scf_iteration[1].energy_total_scf_iteration.magnitude == pytest.approx(-3.86541222e-17)
    assert np.shape(sec_eig.eigenvalues_values) == (1, 2, 5)
    assert sec_eig.eigenvalues_values[0][1][2].magnitude == pytest.approx(8.4504932e-19)
...
def test_relax(parser):
    ...
    assert sec_sccs[3].energy_total.magnitude == pytest.approx(-4.93984603e-18)
    assert sec_sccs[4].section_scf_iteration[1].energy_total_scf_iteration.magnitude == pytest.approx(-2.13640055e-18)
...
def test_dos(parser):
    ...
    assert sec_sccs[0].section_dos[0].dos_energies[70].magnitude == pytest.approx(-3.18261365e-18)
    assert sec_sccs[0].section_dos[0].dos_values[0][151] == pytest.approx(3.5095398518405453e-15)
    assert sec_sccs[1].section_dos[0].dos_values[1][180] == pytest.approx(1.50393389e-14)
    assert sec_sccs[1].section_dos[0].dos_values[1][180] == pytest.approx(1.50393389e-14)
    assert sec_sccs[0].section_dos[0].dos_integrated_values[0][457] == pytest.approx(3.44346629e-29)
    assert sec_sccs[1].section_dos[0].dos_integrated_values[0][1025] == pytest.approx(1.65377776e-28)
